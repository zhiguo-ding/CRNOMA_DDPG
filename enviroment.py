...
class Env_cellular():
    ...
    def step(self, action, state, j):
        ...
        if math.isnan(reward):
            ...
            print(f"the first part of power{(1-alphan)}*{self.eta*self.Pn*hn0/alphan}")
            print(f"alpha is {alphan} and power is {P0n}, {1 - En_bar / (self.T * self.eta * self.Pn * hn0)}")
            print(f"En is {En}, Enbar is {En_bar}, the t*eta*Pn*hn0 is {self.T * self.eta * self.Pn * hn0}")
            print(f"8b is {1 - En_bar / (self.T * self.eta * self.Pn * hn0)}")
            print(f"8c is {1 - (En + En_bar) / (self.T * self.eta * self.Pn * hn0)}")
            print(
                f"8d is {100000*(self.T * self.eta * self.Pn * hn0 - En_bar) / (self.T * self.eta * self.Pn * hn0 + self.T * self.Pmax)}")
            print(f"{(self.T * self.eta * self.Pn * hn0 - En_bar)==0} ")
            print(f"alphan - alphax2={alphan - alphax2}")
            print(f"hn0 is {hn0} and alpha* is {alphaxx}")
            print(
                f"action is {action}, emax-en={self.emax - En},self.T*self.eta*self.Pn*hn0 is {self.T * self.eta * self.Pn * hn0} ")
            print(
                f"the first part is {action * min(self.emax - En, self.T * self.eta * self.Pn * hn0)},  and the second part is {(1 - action) * min(En, self.T * self.Pmax)}")
            print(f"alphaxx = {alphaxx}")
            print(f"alpha01 ={alpha01}")
            print(f"alpha02 ={alpha02}")
            print(f"alphax2 = max(alpha01, alpha02) = {alphax2}")
            print(f"alphan = min(1, max(alphaxx,alphax2)) = {alphan}")
